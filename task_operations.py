...
class Manage():
    ...
    def set_level(self):
        ...
        if (self.number_finished / self.x) >= 0.5 and (self.number_finished / self.x) < 0.7 and (
                self.number_on_going != 0):  # Bronze Level
            db.update({"level": "Bronze", "Score": self.total_score}, users.username == self.username)
            print('Your level is Bronze')

        if (self.number_finished / self.x) >= 0.7 and (self.number_finished / self.x) < 0.8 and (
                self.number_on_going != 0):  # Silver Level
            db.update({"level": "Silver", "Score": self.total_score}, users.username == self.username)
            print('Your level is Silver')

        if (self.number_finished / self.x) >= 0.8 and (self.number_on_going != 0):  # Gold Level
            db.update({"level": "Gold", "Score": self.total_score}, users.username == self.username)
